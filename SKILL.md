...
- 词库内置 **5000 个日常高频词、82 个主题**（2026-09-11：先从 3143/41 扩到 5000/51，当天再把 10 个无语义的「高频补充①~⑩」重构成 41 个按词义命名的主题 → 5000/82），全部 `{cn, en}` 形式 + IPA 音标表 `IPA_MAP` + emoji 映射 `EMOJI_MAP`。

## 数据常量（`<script>` 顶部，两个 IIFE 之外，全局可见）

- `VOCAB` — 主词库 `{ 主题key: {name, icon, words:[{cn,en}...]} }`，**82 个主题**（2026-09-11 主题重构后：19 个原有主题吸纳了语义吻合的新词，其余拆成 41 个新主题 —— 23 个语义主题 + 9 个形容词主题 + 9 个动词主题）。**key 是稳定标识**，改名不改 key；`wordadv_map_stars` 按 key 存星星。
...
- 🔴 **故意不并入 `VOCAB`**：并进去会变成第 83 个主题岛，得改「82 座岛」全套文案、单词卡计数、闯关地图。这层只做查词兜底。**若哪天真要并入 VOCAB，记得同步改 82 岛文案。**
...
**主题折叠**:82 个主题默认只露 9 个(3×3),`.topic-grid.topic-open` 展开,按钮 `#topic-more` 由 `renderTopics()` 动态创建。
...
> ⚠️ 主题重构（51 → 82）时这 4 处又得手动跟着改一遍（`index.html` 的 L1305 / L1394 / L1695 / L1708）。
...
## 🔤 主题重构 51 → 82（2026-09-11，工具在 `tools/reclass/`）
