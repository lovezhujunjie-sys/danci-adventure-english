...
- 词库内置 **5000 个日常高频词、51 个主题**（2026-09-11 从 3143/41 扩到 5000/51），全部 `{cn, en}` 形式 + IPA 音标表 `IPA_MAP` + emoji 映射 `EMOJI_MAP`。

## 数据常量（`<script>` 顶部，两个 IIFE 之外，全局可见）

- `VOCAB` — 主词库 `{ 主题key: {name, icon, words:[{cn,en}...]} }`，51 个主题（41 个原有 + 10 个「高频补充①~⑩」，每主题 186 词，末个 183）。
...
- 🔴 **故意不并入 `VOCAB`**：并进去会变成第 52 个主题岛，得改「51 座岛」全套文案、单词卡计数、闯关地图。这层只做查词兜底。**若哪天真要并入 VOCAB，记得同步改 51 岛文案。**
...
**主题折叠**:51 个主题默认只露 9 个(3×3),`.topic-grid.topic-open` 展开,按钮 `#topic-more` 由 `renderTopics()` 动态创建。
