...
6. **🎧 听力磨耳朵**（顶级模式，整行 featured 卡）— `startListening/lsRun/lsPlay/lsPause`。**免提自动连播**：按选定的「朗读方式」念完一条再走下一个（见下节），列表播完若开「循环播放」则洗牌再来。供开车/跑步/做家务戴耳机用。核心约定：**`lsSpeak()` 是带兜底超时的可等待朗读**（防个别机型 `onend` 不触发卡死队列）；`lsToken` 令牌使暂停/跳词后旧循环立即作废；播放中用 **Wake Lock API 防熄屏**（`lsWakeOn/lsWakeOff` + visibilitychange 回来重新申请），因为锁屏会被系统掐断 speechSynthesis；每听完一句 `recordStudied()` 计入今日/连续天数。控件：⏮ ▶/⏸ ⏭ 大按钮 + `ls-*` 系列 id。UI 有安全提示（开车用车架、锁屏可能停）。

### 🎧 听力三音源：单词 / 短文 / 高频句
听力模式有**三个音源**（`lsSource`）：`word`（单词连播，老逻辑）、`sent`（分级阅读 20 篇按句连听，2026-09-11 加）、`phrase`（**日常高频句 392 句 / 8 场景**，2026-09-17 加）。
- 设置屏「听什么：🔤单词 / 📖短文 / 💬高频句」三片 chips（`#ls-source-opts`）。选短文 → 「选择主题+数量」换成「听哪几篇」（`#ls-book-grid`，21 个按钮 = 全部 + 20 篇，计数是**段数**）；选高频句 → 换成「听哪几个场景」（`#ls-cat-grid`，9 个按钮 = 全部 + 8 场景，计数是**句数**）。`applyLsSource()` 统一管这三套显隐，开始按钮文案跟着变（开始磨耳朵 / 开始听短文 / 开始听高频句）。
- 🔴 **`#ls-cat-grid` 和 `#ls-book-grid` 都必须带 `topic-open` 类**：全局规则 `.topic-grid:not(.topic-open) > .topic-btn:nth-child(n+10)` 会藏掉第 10 个起的按钮。场景现在刚好 8 个卡在边界上，**加到第 9 个就会踩**，所以 `topic-open` 一上来就带着。
- **播放单元 = 一句**，不是单词。`lsWords` 里放 `{en, cn, _topic, _icon, _ipa:'', _book|_cat}`，播放循环本来就只读 `.en`/`.cn`，所以句子对象能直接流过同一套引擎，`lsSpeak/lsWakeOn/循环/停顿/recordStudied` 全部复用。
- 🔴 **三条不能碰的规矩**：
  1. **篇内句子绝不洗牌**。选「全部」时只 `shuffle` 篇目顺序，每篇内部的句子必须保持原文顺序，否则就不是一篇文章了。`lsRun` 里循环重播时，短文走 `lsIdx = 0`（单词和高频句才走 `shuffle(lsWords)`）——这条有单测守着。
  2. **整句不能转小写/拆驼峰**。单词走 `w.en.replace(/([a-z])([A-Z])/g,'$1 $2').toLowerCase()`（为的是 iPhone → i phone），整句这么干会把句首大写和专有名词毁掉。代码里按 `isSent = lsSource !== 'word'` 分支。
  3. **三套播放参数分开存**，键由 `lsKeys()` 管：`word → lsRepeat/lsGap/lsMode`、`sent → lsRepeatSent/lsGapSent/lsModeSent`、`phrase → lsRepeatPhrase/lsGapPhrase/lsModePhrase`。共用一份会出现"在短文里关掉中文，切回单词就没释义了"。
- **高频句音源的取数**：全选 8 场景时 `shuffle` 打乱（否则每次都从「问候寒暄」头几句听起）；只勾一个场景时保持场景内原顺序（那个顺序本身有递进）。场景键失效（存档里留着已删的场景）时**回落到全部**，不能让「开始」点了没反应。
- **入口**：①设置屏 🎧 听力磨耳朵 → 听什么 ②**阅读页底部「🎧 听这篇（磨耳朵）」**（`#rd-listen`）——读完直接听这一篇，会把音源切到短文并选中当前篇，同时掐掉可能正在跑的「从头朗读全文」，免得两个 TTS 抢麦。
- **文案随音源变**：HUD 单位「个/词」（单词）↔「句/句」（短文和高频句），标签「英文读几遍/词间停顿」↔「每句读几遍/句间停顿」，由 `lsSyncChips()` 统一刷。
- **样式**：整句音源（短文/高频句）`.ls-card` 加 `.sent` 类，`.ls-en` 从 38px 收到 21px 并左对齐——一句最长 115 字符，38px 会撑爆卡片。

### 🀄 朗读方式三档（2026-09-17，取代旧的布尔 `lsSayCn`）
老曾原话：「**读英文，再读中文，这样我才能学得懂**」。三档存 `settings.lsMode / lsModeSent / lsModePhrase`：

| 档 | `data-mode` | 读法 |
|---|---|---|
| 🎧 纯英文 | `en` | 英文 ×N，完 |
| 🀄 中英交替（**默认**） | `mix` | 英文 ×N → 中文 → **英文 ×1** |
| 🔁 中英加读 | `mix2` | 英文 ×N → 中文 → **英文 ×N** |

- 🔴 **中文是桥不是终点**：`mix/mix2` 在中文之后**一定要回扣英文**。老曾这个阶段靠中文听懂，但每句都停在中文上，大脑会养成"等翻译"的习惯，真实对话里没人等你翻译。回扣那遍是把意思和声音重新绑一次。
- 🔴 **三档一律默认 `mix`**，包括短文。短文原来是死写 `cn:false`（当年理由"磨耳朵要语流，夹中文就断了"），**2026-09-17 按老曾本人要求改掉**。别再拿"语流"当理由改回纯英文：他现在的阶段听不懂就等于白听。
